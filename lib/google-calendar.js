...
async function getAuthClient() {
  ...
  if (!clientEmail || !privateKey) {
    throw new Error(
      `Google Calendar credentials not configured (email=${!!clientEmail}, key=${!!privateKey})`
    );
  }
  ...
  // Explicitly obtain an access token before returning
  const { token } = await auth.authorize();
  if (!token) {
    throw new Error('JWT authorize() succeeded but returned no access token');
  }
